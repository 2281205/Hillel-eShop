  ...
  if (userInSession) {
    if (localCartId.indexOf(id) !== -1) {
      ...
      document
        ...
        .forEach((e) => (e.className = `product__cart`));
      storageUsers.forEach((item) => {
        if (item.email == userInSession.email) {
      ...
      document
        .querySelectorAll(`button[data-id="${id}"]`)
        .forEach((e) => (e.className = `product__cart product__cart--in`));
      storageUsers.forEach((item) => {
        if (item.email == userInSession.email) {
          item.shoppingCart = userInSession.shoppingCart;
        }
...
const renderItems = ({
  ...
  categories.forEach((item) => {
    let wrap = document.querySelector(`section[data-name=${item}]`)
        ? document.querySelector(`section[data-name=${item}]`)
        : getSection(item),
    ...
    userInSession &&
      localCartId.indexOf(id) !== -1 &&
      btnCart.classList.add("product__cart--in");
    userInSession &&
      localOrderId.indexOf(id) !== -1 &&
      btnCart.classList.add("product__cart--ordered");
    ...
    ImgLike.setAttribute(
      `src`,
      `images/${
        userInSession && userInSession.favourites.indexOf(id) !== -1
      ...
      if (userInSession) {
        if (userInSession.favourites.indexOf(id) !== -1) {
          document
            ...
            .forEach((e) =>
              e.setAttribute(`src`, `images/product__favourite.png`)
            );
          userInSession.favourites.splice(
            userInSession.favourites.indexOf(id),
            1
          );
          ...
          document
            .querySelectorAll(`img[data-id="${id}"]`)
            .forEach((e) =>
              e.setAttribute(`src`, `images/product__favourite--true.png`)
            );
    ...
    priceTotal.innerHTML = `$${
      sale ? price - price * (salePercent / 100) : price
    }`;
...
if (document.querySelector(`#LoginForm`)) {
  ...
  LoginForm.addEventListener(`submit`, (e) => {
    e.preventDefault();
    let email = e.target
        .querySelector(`input[data-name="email"]`)
        .value.toLowerCase(),
  ...
  RegistrationForm.addEventListener(`submit`, (e) => {
    ...
    let name = e.target.querySelector(`input[data-name="name"]`).value,
      email = e.target
        ...
        .value.toLowerCase(),
      password = e.target
        .querySelector(`input[data-name="password"]`)
        .value.trim(),
      passwordVerify = e.target
        .querySelector(`input[data-name="passwordVerify"]`)
        .value.trim(),
      rememberUser = storageUsers.find((user) => user.email === email);
...
if (document.querySelector(`#deleteAcc`)) {
  ...
  deleteAcc.addEventListener(`click`, (e) => {
    e.preventDefault();
    let deleteIndex;
    storageUsers.forEach((item, i) =>
      item.email == userInSession.email ? (deleteIndex = i) : ""
    );
...
if (document.querySelector(`#favouriteTable`)) {
  ...
  caption.innerHTML =
    userInSession.favourites.length > 0
      ? `Favourite Items`
      : ` NO Favourite Items!`;
  ...
  if (userInSession.favourites.length > 0) {
    userInSession.favourites.forEach((item) => {
      ...
      btnCart.className = `product__cart`;
      localCartId.indexOf(favoriteItem.id) !== -1 &&
        btnCart.classList.add("product__cart--in");
      localOrderId.indexOf(favoriteItem.id) !== -1 &&
        btnCart.classList.add("product__cart--ordered");
      ...
      btnLike.addEventListener(`click`, (e) => {
        e.preventDefault();
        userInSession.favourites.splice(
          userInSession.favourites.indexOf(item),
          1
        );
        storageUsers.forEach((item) => {
          if (item.email == userInSession.email) {
      ...
      tdSale.innerHTML = favoriteItem.sale
        ? `<td><span class="item__sale">- ${favoriteItem.salePercent}%</span></td>`
        : `-`;
      tdTotal.innerHTML = `$${
        favoriteItem.sale
          ? favoriteItem.price -
            favoriteItem.price * (favoriteItem.salePercent / 100)
          : favoriteItem.price
      }`;
...
if (document.querySelector(`#orderTable`)) {
  ...
  caption.innerHTML =
    userInSession.orders.length > 0 ? `Ordered Items` : ` NO Ordered Items!`;
  ...
  if (userInSession.orders.length > 0) {
    userInSession.orders.forEach((item) => {
      ...
      tdTotal.innerHTML = `$${
        ordersItem.sale
          ? (ordersItem.price -
              ordersItem.price * (ordersItem.salePercent / 100)) *
            item.count
...
if (document.querySelector(`#shoppingCartTable`)) {
  ...
  caption.innerHTML =
    userInSession.shoppingCart.length > 0
    ...
    userInSession.shoppingCart.forEach((item) => {
      let shopItem = PRODUCTS.find((i) => i.id === item.id),
        ...
        localPrice = shopItem.sale
          ? shopItem.price - shopItem.price * (shopItem.salePercent / 100)
          : shopItem.price;
      ...
      btnDel.addEventListener(`click`, (e) => {
        ...
        document.querySelector(`#orderSummaryTotal`).innerHTML = `$${
          totalPrice.length > 0
  ...
  orderSummary.addEventListener(`submit`, (e) => {
    ...
    if (userInSession.shoppingCart.length > 0) {
      userInSession.shoppingCart.forEach((item) => {
        ...
        userInSession.favourites.splice(
          userInSession.favourites.indexOf(item.id),
          1
        );
      });
